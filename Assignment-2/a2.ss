...
;;All I do is check if the var or triv is a frame variable and
;;then I simply make a displacment-mode operand and return it
...
;;
;;Tail and Body are very similar maybe there is a better way of doing it
;;but I couldnt get it to run for all the test cases
        ...
        (match x
               [(letrec ([,label* (lambda () ,tail*)] ...) ,tail-p)
                (let ((tail (Tail tail-p)) (defn (make-body label* tail*)))
                  `(code ,@tail ,@defn))])))
    ;;We evaluate the tail first because that is how the next pass requires its input
    (define Tail
      (lambda (tail)
        (match tail
               [(begin ,stmt* ... ,tail-p)
...
(define driver
  (lambda (input-program)
    (generate-x86-64 (flatten-program (expose-frame-var (verify-scheme input-program))))))
